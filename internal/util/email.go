...
func SendEmail(body string, recipient string, subject string) error {
	...
	email.SetHeader("From", "zerubabelkassahun116@gmail.com")
	...
	smtpPort := 587
	smtpUsername := "zerubabelkassahun116@gmail.com"
	smtpPassword := "tgew iibg xsch pfht"
    ...
    <body>
        <div class="flex items-center justify-center min-h-screen p-5 bg-gray-600 min-w-screen">
            <div class="max-w-xl p-8 text-center text-gray-800 bg-white shadow-xl lg:max-w-3xl rounded-3xl lg:p-12">
                <h3 class="text-2xl">Thanks for signing up for Tastebite!</h3>
                ...
                <div class="mt-4">
                    <a href={{ .VerificationURL}} class="px-2 py-2 text-blue-200 bg-blue-600 rounded">Click to Verify Email</a>
                    <p class="mt-4 text-sm">If you’re having trouble clicking the "Verify Email Address" button, copy
                        and
                        paste
                        the URL below
                        into your web browser:
                        <a href="" class="text-blue-600">{{ .VerificationURL}}</a>
                    </p>
                </div>
            </div>
        </div>
...
`
	...
	// Create a data structure to pass variables to the template
	data := struct {
		VerificationURL string
	}{
		VerificationURL: "http://localhost:8000/verify-email?token=" + token,
	}

	var bodyBuffer bytes.Buffer

	if err := tmpl.Execute(&bodyBuffer, data); err != nil {
		// Handle error if the template rendering fails
		return "", err
	}
